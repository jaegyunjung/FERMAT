#!/usr/bin/env python3
"""Run Task 19 APOLLO-style Cox survival models for all phenotypes.

This script complements the Task 19 binary classifiers. It keeps the same fixed
index-date framing, but uses censored follow-up rather than requiring complete
5-year observation. Cox coefficients are fit with a ridge penalty in PyTorch so
the Pod does not need an additional survival-analysis package.
"""
...
DEFAULT_OUTPUT_DIR = POD_ROOT / "task19" / "outputs" / "cox_survival_2018_5y_all_phenotypes"
...
APPLICATION_NAME = "fermat_task19_cox_survival"
...
def parse_args():
    ...
    parser.add_argument(
        ...
        nargs="+",
        default=None,
        help="Optional phenotype subset. Default: all eligible label phenotypes.",
    )
...
def build_survival_cache(args, labels: pd.DataFrame, phenotypes: list[str], output_path: Path):
    ...
    with connect(args) as conn:
        with conn.cursor() as cur:
            cur.execute("DROP TABLE IF EXISTS tmp_task19_cox_patient")
            cur.execute(
                """
                CREATE TEMP TABLE tmp_task19_cox_patient (
                    person_id bigint PRIMARY KEY
                ) ON COMMIT PRESERVE ROWS
                """
            )
            cur.executemany(
                "INSERT INTO tmp_task19_cox_patient VALUES (%s)",
                [(int(person_id),) for person_id in patient_ids],
            )
            cur.execute("DROP TABLE IF EXISTS tmp_task19_cox_concept")
            cur.execute(
                """
                CREATE TEMP TABLE tmp_task19_cox_concept (
                    phenotype text,
            ...
            cur.executemany(
                "INSERT INTO tmp_task19_cox_concept VALUES (%s,%s)",
                [
                    (str(row.phenotype), int(row.condition_concept_id))
                    for row in concept_map.itertuples(index=False)
                ],
            )
            cur.execute("CREATE INDEX ON tmp_task19_cox_concept(condition_concept_id)")
            statement = sql.SQL(
                """
                SELECT
                    g.phenotype,
                ...
                JOIN tmp_task19_cox_patient p USING(person_id)
                JOIN tmp_task19_cox_concept g
...
def model_columns(model_set: str, emb_cols: list[str]):
    ...
    for phenotype in phenotypes:
        ...
        for model_set in args.model_sets:
            ...
            if train_events < args.min_train_events or test_events < args.min_test_events:
                ...
                continue
            numeric_cols, categorical_cols, missing_cols = model_columns(model_set, emb_cols)
